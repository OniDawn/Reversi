...
/// @brief simulates a game strategy where the next move is the move, where the most tokens have to be flipped
class REVERSI_EXPORT MinimizePlayer : public Strategy
{
	Q_OBJECT

public:
	static inline const auto Description = QStringLiteral("Setzt den Spielstein an die Stelle, an der die wenigsten Spielsteine umgedreht werden");
	static constexpr auto Name = "Minimize";

	MinimizePlayer();

	/// @brief calculates best move by selecting the fields where the least tokens would be flipped and than chooses randomly
